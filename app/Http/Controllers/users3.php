...
class users extends Controller
{
    public function index()
    {
      ...
      if(Auth::user())
      {
        ...
        {
          return view('company-top', compact('data'));
        }
        else if((Auth::user()->user_category)==1)
        {
          return view('client-top', compact('data'));
        }
      }
      return view('client-top', compact('data'));
    }
    ...
    {
        $user_id = Auth::id();
        $client_order=Order::with(['BidCompany','Bid'])->where('user_id', '=', $user_id)->where('bid_status','<>',2)->where('id','=',$order_id)->get();
